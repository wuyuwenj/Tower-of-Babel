...
export const generate = internalAction({
  ...
  handler: async (ctx, { levelId }) => {
    try {
      ...
      // 3. Mint builds what lives in it. The creature carries the extra
      // constraints because it gets instanced and normalized; the monument
      // stands alone and is fine as composed.
      const [enemyUrl, monumentUrl] = await Promise.all([
        generateModel(spec.enemyPrompt + CREATURE_SUFFIX),
